...
function oneill2Shader(ctx, t, width = 1000, height = 1000) {
    ...
    if (!oneill2Shader.camState) {
        oneill2Shader.camState = {
            dragging: false,
            lastX: 0,
            lastY: 0,
            rotY: 0,
        ...
        canvas.addEventListener('mousedown', e => {
            oneill2Shader.camState.dragging = true;
            oneill2Shader.camState.lastX = e.clientX;
            oneill2Shader.camState.lastY = e.clientY;
        });
        window.addEventListener('mousemove', e => {
            if (oneill2Shader.camState.dragging) {
                const dx = e.clientX - oneill2Shader.camState.lastX;
                const dy = e.clientY - oneill2Shader.camState.lastY;
                oneill2Shader.camState.rotY += dx * 0.012;
                oneill2Shader.camState.rotX += dy * 0.012;
                oneill2Shader.camState.rotX = Math.max(-Math.PI/2, Math.min(Math.PI/2, oneill2Shader.camState.rotX));
                oneill2Shader.camState.lastX = e.clientX;
                oneill2Shader.camState.lastY = e.clientY;
            }
        });
        window.addEventListener('mouseup', () => {
            oneill2Shader.camState.dragging = false;
        });
    }
    const rotY = oneill2Shader.camState.rotY;
    const rotX = oneill2Shader.camState.rotX;
    ...
    for (const face of faces) {
        ...
        const base = [60, 120, 220];
        const color = `rgb(${Math.floor(base[0] + 60 * nl)},${Math.floor(base[1] + 60 * nl)},${Math.floor(base[2] + 40 * nl)})`;
        ctx.beginPath();
    ...
    for (const face of moonFaces) {
        ...
        const color = `rgb(${Math.floor(base[0] * (0.7 + 0.3 * nl))},${Math.floor(base[1] * (0.7 + 0.3 * nl))},${Math.floor(base[2] * (0.7 + 0.3 * nl))})`;
        ctx.beginPath();
        ctx.moveTo(poly[0][0], poly[0][1]);
        for (let k = 1; k < 4; ++k) ctx.lineTo(poly[k][0], poly[k][1]);
        ctx.closePath();
