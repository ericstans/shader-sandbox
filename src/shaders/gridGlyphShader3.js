...
function drawGlyph(ctx, glyph, x, y, scale = 1) {
    ...
    for (let gy = 0; gy < glyph.length; gy++) {
        for (let gx = 0; gx < glyph[0].length; gx++) {
            if (glyph[gy][gx]) {
                // Dry-brush: randomly skip some pixels (more likely at edges)
                let edge = (gx === 0 || gx === glyph[0].length-1 || gy === 0 || gy === glyph.length-1);
                if (!edge && Math.random() < 0.07) continue;
                if (edge && Math.random() < 0.18) continue;
                // Ink bleed: draw faint alpha around filled pixel
                ctx.globalAlpha = 1;
                ctx.fillStyle = '#fff';
                ctx.fillRect(gx * scale, gy * scale, scale, scale);
                // Bleed: draw faint alpha in a 1px ring around
                ...
                for (let dy = -1; dy <= 1; dy++) {
                    for (let dx = -1; dx <= 1; dx++) {
                        ...
                        if (nx >= 0 && nx < glyph[0].length && ny >= 0 && ny < glyph.length) {
                            if (!glyph[ny][nx] && Math.random() < 0.7) {
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < cols; col++) {
            ...
            drawGlyph(
                ctx,
                glyphs[idx],
                offsetX + xOffsets[row][col],
                offsetY + row * (glyphH * scale + space) + baselineY,
                scale
            );
        }
    }
