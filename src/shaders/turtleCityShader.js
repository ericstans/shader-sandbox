...
function animate(ctx, t, width, height) {
    ...
    if (!cachedCity || cachedCity.width !== width || cachedCity.height !== height) {
        ...
        cachedCity = { width, height, cityBaseY, cityW, buildings };
    }

    // Draw city on back (buildings and domes) only below the shell's top edge (masked behind shell)
    ctx.save();
    ctx.beginPath();
    ctx.ellipse(centerX, baseY, shellW / 2, shellH, 0, 0, Math.PI, true); // bottom half of shell as mask
    ctx.clip();
    // Draw buildings
    ...
    // Draw city on back (front view, buildings)
    for (let i = 0; i < cachedCity.buildings.length; i++) {
        let b = cachedCity.buildings[i];
        ctx.save();
        ctx.beginPath();
    ...
    // Draw city details: domes, spires (front view)
    for (let i = 0; i < cachedCity.buildings.length; i++) {
        let b = cachedCity.buildings[i];
        // Dome
        ctx.save();
