// gridPsychedelicCirclesShader2.js
// Shader: Many concentric pastel circles with animated size, thickness, and psychedelic trails (duplicate of Psychedelic Circles 1)
...
function animate(ctx, t, width, height) {
    ...
    for (let i = 0; i < circles.length; i++) {
        const c = circles[i];
        // Animate radius and much thicker lines
        const r = c.baseR * (1 + 0.13 * Math.sin(t * c.freq + c.phase + i * 0.2));
        const thick = c.thickBase + 18 + 12 * Math.sin(t * c.thickFreq + c.thickPhase + i * 0.3);
        // Animate color: cycle hue over time
        const hue = (t * 30 + i * 30) % 360;
        const pastel = `hsl(${hue}, 80%, 80%)`;
        trailCtx.save();
        trailCtx.strokeStyle = pastel;
        trailCtx.lineWidth = Math.max(2, thick);
        trailCtx.shadowColor = pastel;
        trailCtx.shadowBlur = 24 + 12 * Math.sin(t * 0.7 + i);
        trailCtx.beginPath();
